...
DROP TRIGGER IF EXISTS issue_space;
DELIMITER &
...
DROP TRIGGER IF EXISTS acceptIfEditorInChiefCoAuthor;
DELIMITER &
CREATE TRIGGER acceptIfEditorInChiefCoAuthor
AFTER INSERT
ON Submits
FOR EACH ROW
BEGIN
	IF NEW.author IN (
		SELECT email FROM Editor_in_chief)
	THEN 
		UPDATE Article
		SET checked_or_not = 'APPROVED', 
		Approval_Date = current_timestamp
		WHERE Article.article_path = NEW.article_path;
	END IF;
END&
DELIMITER ;

DROP TRIGGER IF EXISTS maxReturnedCopies;
DELIMITER &
CREATE TRIGGER maxReturnedCopies
BEFORE UPDATE
ON Issue
FOR EACH ROW
BEGIN
	IF NEW.Returned_copies > OLD.Printed_Copies THEN
	UPDATE Issue
	SET	  NEW.Returned_copies = OLD.Printed_Copies;
...
DELIMITER &
CREATE PROCEDURE changePassword(emaill VARCHAR(255), new_password VARCHAR(255))
BEGIN 
...
BEGIN
	INSERT INTO Worker VALUES (Name, last_name, email, SYSDATE(), salary, newspaper_name, password, salary,"Journalist");
...
BEGIN
	INSERT INTO Worker VALUES (Name, last_name, email, SYSDATE(), salary, newspaper_name, password, salary,"Administrative");
...
BEGIN
	SELECT password,Attribute FROM Worker 
...
BEGIN
	...
	SET pc = (SELECT Printed_Copies FROM Issue WHERE Issue_no = issueno AND Newspaper_Name = newspaper);
	IF pc < returned_copies THEN 
		SIGNAL SQLSTATE VALUE '23000' 
		SET MESSAGE_TEXT = 'Invalid number';
	ELSE 
	UPDATE Issue
...
DELIMITER $
CREATE PROCEDURE showTotalExpenses(past_date DATE , recent_date DATE)
BEGIN
	DECLARE months INT;
	DECLARE not_found INT DEFAULT 0;
	DECLARE pages_filled INT DEFAULT 0;
	DECLARE Total_expenses INT DEFAULT 0;
	DECLARE recruitment_date DATE;
	DECLARE bcursor CURSOR FOR
	SELECT Date_of_Recruitment FROM Worker WHERE Date_of_Recruitment > past_date;
	...
	REPEAT
			FETCH bcursor INTO recruitment_date;
			IF(not_found=0)
			THEN
				SET Total_expenses = Total_expenses + timestampdiff(month, recruitment_date, recent_date) * ;
			END IF;
		UNTIL(not_found=1)
	END REPEAT;

	SET months = timestampdiff(month, past_date, recent_date);

	SET Total_expenses = months * ( 
		SELECT SUM(salary) FROM Worker
		WHERE Date_of_Recruitment < first_month
		);
...
BEGIN 
	...
	SELECT SUM(salary * timestampdiff(month, greatest(first_month, Date_of_Recruitment), second_month)) INTO all_salary
	FROM Worker
	WHERE Newspaper_Name = paper AND Date_of_Recruitment <= second_month;
...
BEGIN 
	...
	SELECT Date_of_Recruitment FROM Worker WHERE email = em into dor;
	SET paper = (SELECT Newspaper_Name FROM Worker WHERE email = em);

	SELECT sum(Salary * timestampdiff(month, greatest(first_month, dor), second_month)) 
	FROM Worker 
	WHERE email = em INTO all_salary;
	SELECT all_salary;
END&
DELIMITER ; 
